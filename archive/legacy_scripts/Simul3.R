...
Banded <- function(p, diag, offdiag) {
  Sigma_inv <- diag(diag, p)  
  if (p > 1) {
    Sigma_inv[cbind(2:p, 1:(p - 1))] <- offdiag  
    Sigma_inv[cbind(1:(p - 1), 2:p)] <- offdiag  
  }
  return(Sigma_inv)
}
...
########################### Simulation  ###################################
main_function_case1 <- function(n, p, diag, offdiag, lambda_grid = seq(0.01, 2, 0.01), thr = 1.0e-5, num_cores = detectCores() - 1) {
  set.seed(250)
  ...
  BIC_list <- parLapply(cl, lambda_grid, function(lambda) {
    tryCatch({
      result_glasso <- glasso(S, lambda)
      result_dpglasso <- dpglasso.new(S, rho = lambda)
      result_QUIC <- QUIC(S, lambda, msg = 0)
      
      list(
        glasso = BIC_function(result_glasso$wi, S = S, n = n),
        dpglasso = BIC_function(result_dpglasso$X, S = S, n = n),
        QUIC = BIC_function(result_QUIC$X, S = S, n = n)
      )
    }, error = function(e) {
      list(
        glasso = Inf,
  ...
  for (i in seq_along(lambda_grid)) {
    BIC$glasso[i] <- BIC_list[[i]]$glasso
    BIC$dpglasso[i] <- BIC_list[[i]]$dpglasso
    BIC$QUIC[i] <- BIC_list[[i]]$QUIC
  }
  
  Best_lambda <- list(
    glasso = lambda_grid[which.min(BIC$glasso)],
    dpglasso = lambda_grid[which.min(BIC$dpglasso)],
    QUIC = lambda_grid[which.min(BIC$QUIC)]
  )
  ...
  TPR <- list(
    glasso = TPR(solution_glasso, Theta_true),
    dpglasso = TPR(solution_dpglasso, Theta_true),
    QUIC = TPR(solution_QUIC, Theta_true)
  )
  
  FPR <- list(
    glasso = FPR(solution_glasso, Theta_true),
    dpglasso = FPR(solution_dpglasso, Theta_true),
    QUIC = FPR(solution_QUIC, Theta_true)
  )
  
  Sparsity_ratio <- list(
    glasso = Sparsity_ratio(solution_glasso, p),
    dpglasso = Sparsity_ratio(solution_dpglasso, p),
    QUIC = Sparsity_ratio(solution_QUIC, p)
  )
  
  Fnorm <- list(
    glasso = Fnorm(solution_glasso, Theta_true),
    dpglasso = Fnorm(solution_dpglasso, Theta_true),
    QUIC = Fnorm(solution_QUIC, Theta_true)
  )
